
import React from "react";
import { Link } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { ArrowRight, Recycle, Leaf, Zap } from "lucide-react";

const Hero = () => {
  return (
    <div id="home" className="relative overflow-hidden bg-white pt-16">
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-50"></div>
      </div>
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-10 pb-20 md:pt-16 md:pb-28">
        <div className="text-center">
          <h1 className="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
            <span className="block">Transform E-Waste into</span>
            <span className="block eco-text-gradient mt-1">Sustainable Solutions</span>
          </h1>
          <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
            India generates 3.2 million tons of e-waste annually. Join VentureTech's network of kabadiwallas to turn environmental challenges into opportunities for a greener future.
          </p>
          <div className="mt-5 max-w-md mx-auto sm:flex sm:justify-center md:mt-8">
            <div className="rounded-md shadow">
              <Link to="/login?tab=register">
                <Button className="w-full flex items-center justify-center px-8 py-3 text-base font-medium bg-eco-green-600 hover:bg-eco-green-700">
                  Get Started
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
            </div>
            <div className="mt-3 rounded-md shadow sm:mt-0 sm:ml-3">
              <button 
                onClick={() => document.getElementById('how-it-works')?.scrollIntoView({behavior: 'smooth'})}
                className="w-full flex items-center justify-center px-8 py-3 rounded-md text-base font-medium border border-eco-green-600 text-eco-green-600 hover:bg-eco-green-50"
              >
                Learn More
              </button>
            </div>
          </div>
        </div>
        
        <div className="mt-16 relative">
          <div className="relative h-64 overflow-hidden rounded-xl shadow-xl sm:h-72 md:h-80 lg:h-96">
            <div className="absolute inset-0 bg-gradient-to-br from-eco-green-500/90 via-eco-blue-500/90 to-eco-orange-500/90 mix-blend-multiply rounded-xl"></div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center px-6">
                <h2 className="text-2xl font-bold text-white sm:text-3xl md:text-4xl">
                  The Environmental Impact
                </h2>
                <p className="mt-2 max-w-xl text-white/90 text-sm md:text-base">
                  E-waste recycling reduces CO₂ emissions, conserves natural resources, and prevents toxic chemicals from entering our environment
                </p>
                <div className="mt-6 grid grid-cols-3 gap-4 max-w-lg mx-auto">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 transform hover:scale-105 transition-transform">
                    <div className="font-bold text-xl md:text-2xl text-white flex items-center justify-center">
                      <Recycle className="h-5 w-5 mr-1" />
                      <span>70%</span>
                    </div>
                    <div className="text-white/90 text-xs md:text-sm">Recyclable Materials</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 transform hover:scale-105 transition-transform">
                    <div className="font-bold text-xl md:text-2xl text-white flex items-center justify-center">
                      <Leaf className="h-5 w-5 mr-1" />
                      <span>2.5M</span>
                    </div>
                    <div className="text-white/90 text-xs md:text-sm">Tons CO₂ Reduction Goal</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 transform hover:scale-105 transition-transform">
                    <div className="font-bold text-xl md:text-2xl text-white flex items-center justify-center">
                      <Zap className="h-5 w-5 mr-1" />
                      <span>80%</span>
                    </div>
                    <div className="text-white/90 text-xs md:text-sm">Energy Savings</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Hero;
